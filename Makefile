...
ifeq ($(OS),Windows_NT)
    ...
    package_path = nz\ac\auckland\softeng281\hellogithub
    delimiter = \
...
build: clean
	javac -cp $(classpath) -d $(bin) $(src_path)Basics.java $(src_test_path)BasicsTestSuite.java

run: clean build
	java -cp $(classpath) org.junit.runner.JUnitCore $(package).BasicsTestSuite
